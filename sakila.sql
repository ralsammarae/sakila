...
WITH t1 AS (SELECT c.name category_name, NTILE(4) OVER (ORDER BY f.rental_duration) standard_quartile
            FROM film f
            JOIN film_category fc
            ON f.film_id = fc.film_id
            JOIN category c
...
SELECT category_name, standard_quartile, COUNT(*) movie_count
FROM t1
GROUP BY 1, 2
ORDER BY 1, 2;
...
SELECT DATE_PART('month', r.rental_date) rental_month, DATE_PART('year', r.rental_date) rental_year,
       s.store_id, COUNT(r.rental_id) count_rentals
FROM rental r
JOIN staff s
...
WITH t1 AS (SELECT p.customer_id, c.first_name || ' ' || c.last_name full_name, SUM(amount) pay_total
            FROM payment p
            JOIN customer c
...
SELECT DATE_TRUNC('month', p.payment_date) pay_month, t1.full_name, COUNT(*) pay_count, SUM(p.amount) pay_amount
FROM t1
JOIN payment p
